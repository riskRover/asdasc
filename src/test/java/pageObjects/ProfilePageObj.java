...
public class ProfilePageObj extends BaseClass{
	...
	public void removeLocation() {
        // Assuming there's an icon/button to remove the location, we can use XPath to find it.
        // Example: If there is a "Remove" button or "X" next to the selected location "Chennai"
        try {
            // Adjust the XPath based on the actual UI element for the location removal
            WebElement removeLocationButton = driver.findElement(By.xpath("//span[text()='Chennai']/following-sibling::button[text()='Remove']"));
            removeLocationButton.click();
        } catch (Exception e) {
            System.out.println("Could not find remove location button.");
        }
    }
	...
	{
		update_link.sendKeys("E:\\Akshay\\resume.pdf");
	}
